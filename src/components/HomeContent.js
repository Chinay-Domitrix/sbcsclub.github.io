...
function HomeContentHook(Component) {
	return <Component mobile={useMediaQuery({ query: '(max-width: 1000px)' })} />;
}
const workshops = ["PC Building", "Artificial Intelligence", "Cybersecurity", "Game Development", "C", "Web Development", "Python Beginner"],
	leads = [["Hitesh Ale", "Rishi Paladugu"], ["Satvik Vejendla", "Aditi Ajoykumar"], ["Satvik Vejendla", "Aditi Ajoykumar"], ["Yash Nishikant", "Burhan Naveed"], ["Pooja Raghuram", "Sridula Kunamneni", "Tejas Raghuram"], ["Aashika Parekh", "Jeslin Paulraj"], ["Sharvani Vadlamani", "Shreya Venkateswaran"]],
	descriptions = ["Build your own PCs using hardware parts", "Create AI Models for chatbots and health predictors", "Learn how to identify and exploit web vulnerabilities", "Create your own games using JPanel and Pygame", "Learn C from scratch", "Create your own websites using HTML and CSS", "Learn Python from scratch and make your own projects"],
	difficulties = [2, 3, 3, 3, 2, 1, 1]
export default class HomeContent extends Component {
	...
	render() {
		...
		if (document.getElementById("description")) document.getElementById("description").style.opacity = 1 - (500 - this.state.scroll) * (200 - this.state.scroll) / 30000;
		if (this.state.scroll > 190 && !document.getElementById("description").classList.contains("after")) document.getElementById("description").classList.add("transition")
		return (
			<div class="center">
				<div id="home-title">
				...
				<div id="workshops-description" class="center">
					...
					<div id="workshops-list" class="grid">
						{workshops.map((val, i) => <Workshop name={val} leads={leads[i]} description={descriptions[i]} difficulty={difficulties[i]} />)}
					</div>
				</div>
			</div>
		);
