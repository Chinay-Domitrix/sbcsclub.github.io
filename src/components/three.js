...
export default class three extends Component {
	...
	componentDidMount() {
		...
		for (let i = 0; i < objects.length; i++)
			positions.push(Math.random() * 6000 - 3000, Math.random() * 6000 - 3000, Math.random() * 6000 - 3000);
		...
		window.addEventListener("changePhase", () => transition())
		let phase = 1;
		let current = 0;
		let l = 0;
		let p = radiusX - (radiusX - radius) / 2 + 500;
		function transition(duration = 500) {
			for (let i = 0, j = current * particlesTotal * 3; i < particlesTotal; i++, j += 3)
				new TWEEN.Tween(objects[i].position).to({ x: positions[j], y: positions[j + 1], z: positions[j + 2] }, duration).easing(TWEEN.Easing.Exponential.InOut).start();
		}
		camera.lookAt(0, 0, 0)
		let phase1 = 0;
		let sigscale = 800;
		...
		setTimeout(() => {
			for (let i = 0; i < particlesTotal; i++)
				new TWEEN.Tween(objects[i].position).to({ x: positions[i * 3], y: sigscale * 1 / (1 + Math.exp(-positions[i * 3] / 100)) - sigscale / 2, z: positions[i * 3 + 2], }, 200 + i).easing(TWEEN.Easing.Exponential.Out).start();
		}, 1000);
		const animate = () => {
			...
			if (theta_rate > 1 && phase1 != 2) {
				...
				setTimeout(() => {
					for (let i = 0; i < objects.length; i++) {
						...
						positions.push(1000 * Math.cos(th) * Math.sin(ph), 1000 * Math.sin(th) * Math.sin(ph), 1000 * Math.cos(ph))
					}
					current = 1;
					positions.reverse()
					finishedanim = true;
					window.dispatchEvent(new CustomEvent("finishedAnimation"))
				}, 400)
			}
			if (phase1 == 1) {
				for (let i = 0; i < objects.length; i++) {
