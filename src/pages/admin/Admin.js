...
import { Grid, Card, CardContent, CardActions, CardHeader, Typography, IconButton, Button } from "@material-ui/core";
...
class Admin extends Component {
	...
	fetchWorkshops = () => firebase.database().ref("workshops").once("value").then((snapshot) => {
		if (snapshot.val())
			this.setState({ workshops: snapshot.val() });
	});
	updateWorkshops = () => firebase.database().ref("workshops").set(this.state.workshops);
	newEvent = () => {
			...
			<div className="outer-admin">
				<Grid container spacing={3} style={grid}>
						...
						if (val.created)
									...
									<Card key={i}>
										<CardHeader action={
											<IconButton onClick={() => this.removeWorkshop(i)}>
												<DeleteIcon />
											</IconButton>} title={val.name} />
										<CardContent key={i}>
											<Typography variant="subtitle2" color="textSecondary" gutterBottomkey={`${i}.2`}>
												{val.lead}
											</Typography>
										</CardContent>
									</Card>
							...
							return (
								<Grid item xs={3} key={i}>
									<Card key={i}>
										<CardContent key={i}>
											<Input hint="Workshop Name" ref={(c) => { this.name = c; }} />
											<SubInput hint="Name" ref={(c) => { this.lead = c; }} />
										</CardContent>
					...
					<Grid item>
						<Card style={section}>
							<CardActions>
								<IconButton size="small" color="inherit" onClick={this.newEvent}>
...
export default class App extends Component {
	...
	render() {
		if (this.state.admin === 0)
			if (cookies.get("fuid"))
				firebase.database().ref("members").child(id).once("value").then((snapshot) => {
